    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                query = string.Format("select * from da where id_data = '{0}'", TxtNIM.Text);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        TxtLastName.Text = kolom["lastname"].ToString();
                        TxtFirstName.Text = kolom["Fist_name"].ToString();
                       CBProdi.Text = kolom["Jurusan"].ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                koneksi.Open();
                query = string.Format("update da set lastname = '{0}', Fist_name = '{1}', semester = '{2}', Jurusan = '{3}' where id_data ='{4}'", TxtLastName.Text, TxtFirstName.Text, CBProdi.Text, TxtNIM.Text);
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
                perintah.ExecuteNonQuery();
                ds.Clear();
